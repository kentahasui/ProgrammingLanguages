...
(define arith-op-tests
  (test-suite
   "tests for arithmetic operators: exercise 3.7"
   
   ;; addition
   (check-equal? (run "+(44,33)") 77 "addition 1")

   ;; multiplication
   (check-equal? (run "*(4,3)") 12 "multiplication 1")

   ;; integer quotient
   (check-equal? (run "/(44,11)") 4 "integer quotient 1")   
))

(define comp-op-tests
  (test-suite
   "tests for numeric comparison operators: exercise 3.8"
   
   ;; equal?
   (check-equal? (run "equal?(42,42)") #t "equal? 1")
   (check-equal? (run "equal?(42,43)") #f "equal? 2")

   ;; less?
   (check-equal? (run "less?(4,3)") #f "less? 1")
   (check-equal? (run "less?(3,4)") #t "less? 2")

   ;; greater?
   (check-equal? (run "greater?(11,44)") #f "greater? 1")   
   (check-equal? (run "greater?(44,11)") #t "greater? 2")   
))

(define cond-tests
  (test-suite
   "tests for cond expression: exercise 3.12"
   
   ;; cond
   (check-equal? (run "cond equal?(42,42) ==> 2
                            less?(43,43)  ==> 3 end") 
                 2 
                 "cond 1")
   (check-equal? (run "cond equal?(42,43) ==> 2
                            less?(42,43)  ==> 3 end") 
                 3 
                 "cond 2")
   (check-exn exn?
              (lambda () (run "cond equal?(42,43) ==> 2
                                    less?(43,43)  ==> 3 end"))
              "cond none true 1")
...
(define allof-tests
  (test-suite
   "tests for allof expression"
   
   ;; allof
   (check-equal? (run "allof zero?(0) end") 
                 #t 
                 "allof 1")
   (check-equal? (run "if allof zero?(0) end then 3 else 4") 
                 3 
                 "allof 2")
   (check-equal? (run "let x = 0 in let y = 1 
   ...
   (check-exn exn? 
              (lambda () 
                (run "let f = proc (x) aha   % raises error if called
...
(run-tests arith-op-tests 'verbose)
(run-tests comp-op-tests 'verbose)
